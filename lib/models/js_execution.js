...
JSExecution.prototype._step = function () {
  ...
  next = this._sequence[0]
  return next ? next[2] : null
}

JSExecution.prototype.run = function () {
  if (this._sequence) {
    while (this._sequence.length) this._step()
    this.trigger('end')
  } else {
    try {
      run(this.code, this.globals)
    } catch (exc) {
