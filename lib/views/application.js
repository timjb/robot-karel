Karel.Views.Application = Skin.extend({
  ...
  render: function () {
    ...
    this.editorToolbar.bind('show-structogram', _.bind(function () {
      new Skin.Overlay({
        header: "Structogram",
        body: new Karel.Views.Structogram({ model: this.model })
      }).appendTo(this.el)
    }, this))
    
    this.mainToolbar.bind('show-new-world-dialog', _.bind(function () {
      var proto = new Karel.Models.World()
      var overlay = new Skin.Overlay({
        header: "New World",
        className: 'new-world-overlay',
        body: new Skin.Form({
            model: proto,
            fields: [
        ...
        footer: new Skin.OverlayFooter({ primary: "Create" })
          .bind('primary', _.bind(function () {
            overlay.close()
            this.model.set({
          ...
          .bind('secondary', _.bind(function () {
            overlay.close()
          }))
      }).appendTo(this.el)
