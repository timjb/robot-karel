...
Karel.Views.World3D = Karel.Views.WorldBase.extend(!Skin.hasCanvasSupport ? {} : {
  ...
  initialize: function (options, povOptions) {
    ...
    var numberOrDefault = function (a, b) { return typeof a === 'number' ? a : b }
    this.degrees = numberOrDefault(povOptions.degrees, 45)
    this.cameraZ = numberOrDefault(povOptions.cameraZ, 120)
    this.radius  = numberOrDefault(povOptions.radius,  400)
  ...
  createGrid: function () {
    ...
    // Left Side
    var plane = new THREE.Mesh(new THREE.PlaneGeometry(h*GH, d*GW, h, d), material)
  ...
  updateField: function (x, y, field) {
    ...
    while (field.bricks > fieldObj.bricks.length) {
      ...
      var cube = new THREE.Mesh(
        new THREE.CubeGeometry(GW, GW, GH, 1, 1),
    ...
    if (field.block && !fieldObj.block) {
      var cube = new THREE.Mesh(
        new THREE.CubeGeometry(GW, GW, 2*GH, 1, 1),
        new THREE.MeshLambertMaterial({
  ...
  createCamera: function (width, height) {
    this.camera = new THREE.Camera(50, width/height, 1, 1e5)
