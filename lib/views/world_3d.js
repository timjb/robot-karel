  ...
  updateField: function(x, y, field) {
    ...
    while (field.bricks > fieldObj.bricks.length) {
      ...
      var cube = new THREE.Mesh(
        new THREE.Cube(GW, GW, GH, 1, 1, new THREE.MeshLambertMaterial({ color: settings.COLORS.BRICK, shading: THREE.FlatShading })),
    ...
    if (field.block && !fieldObj.block) {
      var cube = new THREE.Mesh(new THREE.Cube(GW, GW, 2*GH, 1, 1), new THREE.MeshLambertMaterial({ color: settings.COLORS.BLOCK, shading: THREE.FlatShading }))
      cube.position.x = GW/2 + x0 + x*GW
      cube.position.y = -GW/2 + y0 - y*GW
      cube.position.z = GH
      scene.addObject(cube)
