...
n.FunctionInvocation.prototype.eval = function (env, cb) {
  ...
  return attach(self, function () {
    // EVAL -> APPLY
    var func   = self.identifier.eval(env)
    ,   params = self.parameters.eval(env)
    if (typeof func === 'undefined') {
      throw new Error("Function '"+self.identifier.name+"' is not defined.")
    }
    return apply(func, params, cb)
  })
}
...
n.Parameters.prototype.eval = function (env) {
  log("Parameters")
  return _.map(this.list, function (node) {
...
var apply = function(fn, parameters, cb) {
  if (typeof fn === 'function') { // A primitive function.
    ...
    // Bind the formal parameters to their values
    _.each(def.formalParameters.list, function (identifier, i) {
      env[identifier.name] = parameters[i]
    });
